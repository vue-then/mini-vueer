					...
					>
						<ul>
							<li class="pk-1px-b" v-for="(item, index) of list" :key="index">
								<div class="title" :class="{'green':item.outStatus === 2,'red':item.outStatus === 4}">
									<p>
										<span>提现</span>
										<span class="status">{{item.statusName}}</span>
									</p>
									<span class="money">{{item.withdrawalMoney | currency('￥',2)}}</span>
								</div>
								<div class="order">
...
<script>
import { getWithdrawList } from "@/services/moneyRecord";
